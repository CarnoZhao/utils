...
def dice_metric(true, logits, eps=1e-7, thres = 0.5):
    """Computes the Sørensen–Dice loss.
    Note that PyTorch optimizers minimize a loss. In this
    case, we would like to maximize the dice loss so we
    return the negated dice loss.
    ...
    if num_classes == 1:
        ...
        true_1_hot_s = true_1_hot[:, 1:2, :, :]
        true_1_hot = torch.cat([true_1_hot_s, true_1_hot_f], dim=1)
        pos_prob = (torch.sigmoid(logits) > thres).float()
        neg_prob = 1 - pos_prob
        probas = torch.cat([pos_prob, neg_prob], dim=1)
    else:
        true_1_hot = torch.eye(num_classes)[true.squeeze(1)]
...
def iou_loss(true, logits, eps=1e-7):
    """Computes the Sørensen–Dice loss.
    Note that PyTorch optimizers minimize a loss. In this
    case, we would like to maximize the dice loss so we
    return the negated dice loss.
    ...
    intersection = torch.sum(probas * true_1_hot, dims)
    cardinality = torch.sum(probas + true_1_hot, dims)
    iou_loss = (intersection / (cardinality - intersection + eps)).mean()
    return (1 - iou_loss)
...
class DiceLoss(torch.nn.Module):
    ...
    def forward(self, logits, target):
        return dice_loss(target, logits)

class DiceMetric(torch.nn.Module):
    def __init__(self):
        super(DiceMetric, self).__init__()

    def forward(self, logits, target, search = False):
        return dice_metric(target, logits, 0.5)
...
class SqueezeBCELoss(torch.nn.Module):
    def __init__(self):
        super(SqueezeBCELoss, self).__init__()
